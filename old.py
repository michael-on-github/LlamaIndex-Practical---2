...
llm = LlamaCPP(
    model_path=os.path.expanduser("./openhermes-2.5-mistral-7b.Q6_K.gguf"),
    temperature=0,
    max_new_tokens=512,
    context_window=4096,
    verbose=False,
    ...
    if os.path.exists(CANDIDATES_ROSTER_PATH):
        ...
        for i, (file_path, chunks) in enumerate(grouped_chunks.items()):
            ...
            prompt = f"""
                ...
                Do NOT include summary. do NOT include metadata. Output _only_ _valid_ JSON with the following keys:
            ...
            candidates_roster[file_path] = json.loads(response.text)
            progress_text.text(
